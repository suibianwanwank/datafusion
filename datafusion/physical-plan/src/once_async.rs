...
//! Shared asynchronous coordination primitives used by multiple execution plans.
...
/// the hash table to be built before proceeding, so they all wait on the same
/// [`OnceAsync`] instance.
...
mod tests {
    ...
    use datafusion_common::{arrow_datafusion_err, arrow_err};
